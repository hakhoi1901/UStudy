...
- Đề xuất cho mỗi môn có thể chỉnh:  
  ...
  Nếu `editableCredits === 0` thì không còn môn để đề xuất. Clamp `suggestedGrade` trong [0, 10].
...
2. Với mỗi môn **không** khóa:  
   - `suggestedGrade = remainingPoints / editableCredits` (nếu `editableCredits > 0`), clamp [0, 10].  
...
- [ ] Sinh viên nhập GPA mục tiêu (vd 8.0) → thấy GPA hiện tại, mục tiêu, tín chỉ còn lại, điểm TB tối thiểu cần đạt.
- [ ] Thấy “GPA theo kỳ”: ít nhất một kỳ (Học kỳ tiếp theo) với mức GPA cần đạt = requiredAverage.
- [ ] Bảng môn trong kỳ: mỗi môn có đề xuất điểm (ban đầu = requiredAverage), có thể nhập “điểm dự kiến”.
- [ ] Môn đã có điểm (từ DB hoặc khóa) → read-only; đề xuất các môn còn lại để đạt GPA kỳ.
- [ ] Khi user sửa điểm một môn (vd A=10, B=8.5), các môn còn lại (C, D) tự cập nhật đề xuất sao cho GPA kỳ = requiredAverage; GPA tổng vẫn đạt mục tiêu (trong phạm vi một kỳ).
- [ ] Không ảnh hưởng Mô phỏng GPA và các trang khác; build không lỗi; logic GPA thống nhất với GPACalculator/AcademicRulesEngine.
