...
import type { DashboardCalendarEvent } from './dashboard-calendar-events';
...
export function DashboardCalendarWidget({
  ...
  return (
    <section className="flex max-h-[420px] min-h-[280px] min-h-0 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white">
      {/* Header */}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 [scrollbar-width:thin]">
          <div>
            {groupedEvents.map((group, index) => (
              <section key={group.key} className={index === 0 ? "pt-2" : "py-2"}>
                {/* Date heading */}
                <div className="sticky top-0 z-10 mb-2 flex items-center justify-between bg-white/95 py-2 backdrop-blur">
                  <div className="flex items-baseline gap-2">
                    <h4 className="text-xs font-bold text-[#004A98]">
                ...
                <div className="overflow-hidden rounded-lg border border-gray-200">
                  {group.events.map((event) => {
                    ...
                    return (
                      <article
                        key={event.id}
                        className="group flex items-start gap-3 border-b border-gray-200 px-3 py-3 transition-colors last:border-b-0 hover:bg-gray-50"
                      >
                        <div
                          className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${isClass
                              ? "ustudy-icon-primary-soft"
                              : "ustudy-icon-success-soft"
                            }`}
                        >
