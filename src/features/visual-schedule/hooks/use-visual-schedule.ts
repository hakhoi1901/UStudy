...
export function useVisualSchedule({ selectedSemester }: UseVisualScheduleProps = {}) {
    ...
    const totalWeeks = useMemo(() => {
        const scheduledWeeks = Math.max(0, ...schedule.sessions.map((session) => session.totalWeeks || 0));
        const datedWeeks = semesterStartDate
            ? Math.max(0, ...schedule.sessions.map((session) => {
                if (!session.endDateParsed) return 0;
                return Math.floor((session.endDateParsed.getTime() - semesterStartDate.getTime()) / (7 * 24 * 60 * 60 * 1000)) + 1;
            }))
            : 0;
        const baseWeeks = Math.max(scheduledWeeks, datedWeeks, 1);
