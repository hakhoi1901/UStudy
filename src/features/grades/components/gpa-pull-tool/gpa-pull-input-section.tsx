        ...
        <>
            {isGuidanceActive ? (
                <>
                    ...
                    <p className="mt-1 text-xs leading-4 text-gray-600 sm:text-sm">Sau kỳ còn cần: <span className="font-semibold tabular-nums text-gray-800">{semesterStats?.newRequiredAvgAfter?.toFixed(decimals) ?? '-'}</span></p>
                </>
            ) : (
                <>
                    <p className="text-[11px] font-medium leading-4 text-gray-500 sm:text-xs">GPA {scopeName.toLowerCase()} dự kiến</p>
                    <p className="mt-0.5 text-2xl font-bold tabular-nums text-[#004A98] sm:text-3xl">{projectedScopeGPA.toFixed(decimals)}</p>
                    <p className="mt-1 text-xs leading-4 text-gray-600 sm:text-sm">Đã nhập điểm: <span className="font-semibold tabular-nums text-gray-800">{projectedScopeCredits} TC</span></p>
                </>
            )}
        </>
    );
    ...
    return (
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0 flex-1 space-y-1">
                <div className="grid grid-cols-[minmax(0,0.8fr)_minmax(112px,0.72fr)] gap-3 pb-3 lg:block lg:pb-0">
                    <div className="flex min-w-0 flex-col items-start gap-2 lg:flex-row lg:flex-wrap lg:items-center">
                        ...
                        <div className="flex min-w-0 flex-1 flex-wrap items-start gap-2">
                            <input
                                ...
                                className={`w-full max-w-28 rounded-lg border bg-gray-50 px-3 py-2 text-sm font-semibold tabular-nums focus:border-transparent focus:outline-none focus:ring-2 lg:w-24 ${targetGpaError
                            ...
                            <div className="desktop-only">
                                {GPA_CONFIG.slice(0, 4).map((config) => (
                                    <button
                                        ...
                                        className="min-w-10 rounded-lg border border-gray-200 bg-white mx-1 px-2 py-2 text-sm font-medium text-gray-700 transition-colors hover:border-[#004A98] hover:bg-[#EAF3FF] hover:text-[#004A98]"
                                    >
                                        {config.value}
                                    </button>
                                ))}
                    ...
                    <div className="col-span-2 md:p-2 lg:pl-24">
                        {targetGpaError ? (
                            <p className="text-xs font-medium leading-5 text-red-600 sm:text-sm" role="alert" aria-live="polite">
                                {targetGpaError}
                            </p>
                        ) : mode !== 'currentSemester' && maxTargetGpa !== null ? (
                ...
                <div className="flex flex-col items-stretch gap-2 lg:flex-row lg:flex-wrap lg:items-center">
                    <span className="text-sm font-medium text-gray-700 lg:w-24 lg:pt-2">Phạm vi</span>
                    <div className="grid w-full grid-cols-3 rounded-lg border border-gray-200 bg-gray-50 p-1 lg:flex lg:w-auto">
                        ...
                        <button
                            ...
                            className={`min-w-0 rounded-md px-2 py-1.5 text-xs font-semibold transition-colors lg:px-3 ${mode === scope.id ? 'bg-white text-[#004A98] shadow-sm' : 'text-gray-500 hover:text-gray-700'} disabled:cursor-not-allowed disabled:opacity-50`}
                        >
                            {scope.label}
                        </button>
                        ))}
                    ...
                    {isFoundationMajorModeUnavailable && <p className="w-full text-xs text-amber-700 lg:pl-24">Chưa có dữ liệu danh mục để tính riêng Cơ sở ngành.</p>}
                    <p className="w-full text-xs leading-5 text-gray-500 lg:pl-24">Bạn có thể nhập điểm dự đoán trực tiếp ở bảng bên dưới mà không cần bật gợi ý.</p>
                </div>
            </div>
